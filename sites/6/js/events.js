...
clickHandler = function(event) {
  ...
  if (intersects.length > 0) {
    if (intersects[0].object.type !== 'cylinder') {
      ...
      control.object.material.emissive.set(0x444444);
      // control.object.fixedFloor = $('#fixedFloorDistanceChecker')[0].checked;
      // control.object.fixedFloorDistance = $('#fixedFloorDistance').val();
      setObjectName();
  ...
  if (intersects.length > 0) {
    ...
    if (rightclick) {
      ...
      if (control.object.name === 'hole'){
        ...
        if (control.object.holeType === 'square') {
  ...
  */
    ...
    C.forEach(function(c,i){
      OBJECTS.forEach(function(o,_i){
        if (o.uuid === c.uuid){
          o.scaleArrows.forEach(function(a){
            scene.remove(a);
          });
    ...
    var merge = Merge(C);
    var singleGeometry = merge[0];
    var holesGeometry = merge[1];

    var mesh = new THREE.Mesh(singleGeometry, C[0].material);
    var holes = new THREE.Mesh(
      holesGeometry,
    ...
    b1 = new THREE.Box3().setFromObject(cc);
    b2 = new THREE.Box3().setFromObject(holes);


    // holes.position.y = cc.rect.y/2+0.001;
    // holes.rotation.x = Math.PI/2;
    // holes.position.x += cc.rect.x/6;
    // holes.position.x += (b1.max.x - b2.max.x)/2;
    // holes.position.z -= cc.rect.z/6 ;
    // holes.position.z -= (b1.max.z - b2.max.z)/2;
    // holes.position.z -= cc.rect.z/2;
    // cc.add(holes);
...
$('.subcategories ul li').click(function(e) {
  ...
  return object = new OBJ($(this).data('objectmodel'), size, $('[data-obj-texture].active').data('obj-texture'), function(cube) {
    ...
    control.object.fixedFloor = $('#fixedFloorDistanceChecker')[0].checked;
    control.object.floorDistance = $('#objectSpaceHeight').val();
    ...
    TIMES.forEach(function(t, i) {
      ...
      if (cube.timestamp > t[0] && cube.timestamp < t[1]) {
        cube.position.y = -room.rect.y / 2 + cube.rect.y / 2 + parseFloat($('#objectSpaceHeight').val() / 100);
      }
    });
    setObjectName();
    enableObjectControls();
...
controlsUpdate = function(pos, event) {
  var box;
  ...
  if (CACHE.length <= 1 && event.ctrlKey) {
...
$('.close-popup, [data-close]').click(function(e) {
  e.preventDefault();
  $('.popup').removeClass('active');
  // controls.enabled = true;
  isContextMenu = false;
...
updateRoomProperties = function() {
  room._updateRect({
    x: parseFloat($('#roomLengthInput').val()/1000),
    z: parseFloat($('#roomWidthInput').val()/1000),
    y: parseFloat($('#roomHeightInput').val()/1000)
  });
...
$('#objectSpaceHeight').keyup(function() {
  ...
  setTimeout(function() {
    // if (control.object){
      var val = parseInt($($t).val())/10 || 0;
      $('#objectSpaceHeight2').val(val)
...
$('#objectSpaceHeight2').on('input', function() {
  // if (control.object){
    var val = $(this).val()/10 || 0;
    $('#objectSpaceHeight').val(val);
    control.object.floorDistance = val;
  // }
});
...
$('#objectDistanceLeft, #objectDistanceFront, '+
  ...
  .each(function(i, input){
    $(input).on('keyup', function(e){
      ...
      if ($(this).attr('id') === 'objectDistanceBottom'){
        control.object.floorDistance = $(this).val()/100;
      }
    });
  });
...
$('#save').click(function(e){
  ...
  $.ajax({
      url: 'http://185.31.163.202/editor/save.php',
      type: 'POST',
      data: json,
...
$('#load').click(function(e){
  ...
  $.ajax({
      url: 'http://185.31.163.202/editor/scene.json',
      type: 'GET',
      dataType: 'json',
      async: true,
      complete: function(json) {
        $('[data-close="loadPopup"]').click();
        if (hole){
          hole.visible = false;
        }
...
$('#booleanOperaion').click(function(e){
  e.preventDefault();
  hole.create(control.object, OBJECTS, scene);
});
$('#addHoleObj').click(function(e){
  e.preventDefault();
  if (hole){
    scene.remove(hole);
  }
  hole = new Hole(control.object);
});

// $('#makeHole').click(function(e){
//   e.preventDefault();
//   $('#createHolePopup').removeClass('active');
...
$('#holeType').on('change', function(){
  if (hole){ scene.remove(hole) }
  hole = new Hole(control.object);
});
$('#holeSize').on('change keydown keyup', function(){
  if (hole){ scene.remove(hole) }
  hole = new Hole(control.object);
});
$('#hideHole').click(function(e){
  e.preventDefault();
  control.detach();
  hole.visible = false;
});